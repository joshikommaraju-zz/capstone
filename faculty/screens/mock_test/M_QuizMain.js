...
import {getSubjectByCode} from '../../graphql/queries.js';
import { updateClassSection } from '../../graphql/mutations.js';
...
class M_QuizMain extends Component{
  ...
  async componentDidMount(){
    ...
    console.log(this.props.Testname,"Oyyyyyyyyyyyy")
    let temp=[]
  ...
  post(){
    this.addQ();
    var abc=""
    abc=abc+"{'subjectCode':'"+this.props.subjectCode+"','heading':'"+this.props.Testname+"','date':'"+this.props.date+"','duration':'"+this.props.duration+"','totalmarks':'"+this.props.maxMark+"','totalQues':'"+this.props.questions+"','marknegative':'True','questions':["
    for(var i=1;i<=this.props.questions;i++){
    ...
    console.log("Test here",abc);
    this.upPost(abc);
  }

  async upPost(data){
    let subClass = await API.graphql(graphqlOperation(getSubjectByCode, {subjectCode: this.state.subjectCode}));
    let classId = subClass.data.getSubjectByCode.classSection.id
    let tests = subClass.data.getSubjectByCode.classSection.mockTests
    let ftest = tests.concat(data)
    console.log(classId,ftest)
    await API.graphql(graphqlOperation(updateClassSection, {input:{id:classId,mockTests:ftest}}));
    Actions.popTo('home')
  }
...
<View style = {
      {"alignItems": "center","justifyContent":"space-between","paddingTop": width*0.12,
        "flexDirection":"row",
        paddingLeft:"9%",
        paddingRight:"10%",
        width:"100%",
