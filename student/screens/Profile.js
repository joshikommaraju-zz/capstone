...
class profile extends Component{
  ...
  async componentDidMount(){
    ...
    let DBd = JSON.parse(DBD)
    let subData = this.props.subData;
    this.getToken();
    let user = DBd.name
    let rollNum = DBd.rollnum
    let classx = DBd.class
    let section = DBd.section
    let school = DBd.school.schoolName
    this.setState({
      rollNum:rollNum,
      SchoolName: school,
      classx: classx,
 ...
 async getToken(user) {
  ...
  return (
    ...
    <View style = {
      {alignItems:"center",height:"15%",marginTop:"5%", width:"100%",justifyContent:"space-around"}} >
    ...
    } > Class: {this.state.classx}-{this.state.section} . Roll: {this.state.rollNum} </Text>
    <Text style = {
      {
        "fontFamily": "Poppins-Medium",
        "fontWeight": "400",
